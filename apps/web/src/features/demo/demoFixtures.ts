...
const demoAuthFiles: AuthFilesResponse = {
  ...
  files: [
    ...
    },
    {
      ...
      account: 'AG Claude/GPT Pool',
      label: 'Antigravity 3P Pool',
      account_snapshot: 'AG Claude/GPT Pool',
      priority: 35,
      success: 492,
      failed: 9,
...
const buildMonitoringAnalytics = (
  ...
  const accountStats = [
    ...
    },
    {
      ...
      account_snapshot: 'AG Claude/GPT Pool',
      auth_label_snapshot: 'Antigravity 3P Pool',
      auth_provider_snapshot: 'antigravity',
...
const demoInspectionRunDetail = (baseNow = now()): CodexInspectionRunDetail => ({
  ...
  results: [
    ...
    },
    {
      ...
      state: 'active',
      action: 'reauth',
      actionReason: 'HTTP 401 from quota refresh',
  ...
  logs: [
    {
      ...
      runId: 1001,
      level: 'info',
      message: `Inspection completed for ${getDemoAuthFileItems().length} auth files`,
...
export const getDemoAuthFiles = (): AuthFilesResponse => {
  const files = getDemoAuthFileItems();
  return clone({ total: files.length, files });
};
        ...
        },
        {
          ...
          buckets: [
            ...
            },
            {
              ...
              resetTime: demoResetIso(5 * day),
              description: 'Weekly Claude/GPT pool is close to the low-quota threshold',
            },
          ],
        },
      ],
    ...
    body = {
      ...
      rate_limit: {
        ...
        primary_window: {
          ...
          reset_at: Math.floor((now() + 8280 * 1000) / 1000),
        },
        secondary_window: {
          used_percent: 42,
          limit_window_seconds: 604800,
          reset_after_seconds: 246000,
          reset_at: Math.floor((now() + 246000 * 1000) / 1000),
        },
      },
      code_review_rate_limit: {
        ...
        primary_window: {
          ...
          reset_at: Math.floor((now() + 7200 * 1000) / 1000),
        },
        secondary_window: {
          used_percent: 24,
          limit_window_seconds: 604800,
          reset_after_seconds: 360000,
          reset_at: Math.floor((now() + 360000 * 1000) / 1000),
        },
      },
      additional_rate_limits: [
        {
          limit_name: 'codex_review',
          rate_limit: {
            allowed: true,
            primary_window: {
              used_percent: 18,
    ...
    body = {
      config: {
        currentPeriod: {
          ...
          end: new Date(now() + 5 * day).toISOString(),
        },
        ...
        monthlyLimit: { val: 100_000 },
        used: { val: 86_000 },
        onDemandCap: { val: 50_000 },
        onDemandUsed: { val: 12_000 },
        billingPeriodStart: new Date(now() - 15 * day).toISOString(),
        billingPeriodEnd: new Date(now() + 15 * day).toISOString(),
      },
    };
