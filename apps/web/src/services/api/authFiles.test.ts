...
describe('authFilesApi setStatusWithFallback', () => {
  it('prefers /auth-files/status over /auth-files for disable', async () => {
    ...
    await authFilesApi.setStatusWithFallback({ name: 'xai.json' }, true);
  ...
  it('rejects non-plugin-virtual status endpoint failures', async () => {
    mocks.patch.mockRejectedValueOnce(new Error('status endpoint missing'));

    await expect(
      authFilesApi.setStatusWithFallback({ name: 'xai.json' }, true)
    ).rejects.toThrow('status endpoint missing');
