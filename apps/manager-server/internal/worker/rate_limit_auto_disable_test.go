...
func TestQuotaAutoDisableCandidateRejectsXAINonFutureExplicitReset(t *testing.T) {
	...
	// Non-future explicit timestamps are ignored; free-usage falls back to the default 24h window.
	candidate, ok := quotaAutoDisableCandidateFromEvent(event, "http://cpa", "key", now)
	if !ok {
		t.Fatal("free-usage signal should still create a candidate via the 24h fallback")
	}
	if got, want := candidate.ResetAt, now.Add(24*time.Hour); !got.Equal(want) {
		t.Fatalf("reset time = %s, want %s", got, want)
	}
