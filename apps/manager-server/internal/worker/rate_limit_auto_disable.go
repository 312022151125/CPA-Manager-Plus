...
const (
	...
	xaiFreeUsageCooldown          = xaiFreeUsageExhaustedCooldown
...
func xaiFreeUsageExhaustedResetTimeFromEvent(event usage.Event, now time.Time) (time.Time, bool) {
	...
	if providerUsage := xaiProviderUsageFromEvent(event, now); providerUsage != nil && strings.EqualFold(providerUsage.Code, usage.ProviderUsageCodeXAIFree) {
		...
		if resetAt, ok := xaiFreeUsageResetTimeFromTexts(texts, observedAt); ok && resetAt.After(now) {
			return resetAt, true
		}
		if event.ResponseMetadata == nil || event.ResponseMetadata.Errors == nil || event.ResponseMetadata.Errors.RetryAfterSeconds == nil {
		...
		resetAt := observedAt.Add(xaiFreeUsageExhaustedCooldown)
...
var (
	...
	xaiRelativeResetKeys = []string{"reset_after_seconds", "resetAfterSeconds", "resets_in_seconds", "resetsInSeconds"}
)
...
func isXAIFreeUsageProvider(event usage.Event) bool {
	rawProvider := strings.ToLower(strings.TrimSpace(event.Provider))
	return (normalizeQuotaAutoDisableProvider(event.Provider) == "xai" && rawProvider != "grok") || usage.IsNativeXAIProvider(event.Provider, event.AuthProviderSnapshot, event.ExecutorType)
}
