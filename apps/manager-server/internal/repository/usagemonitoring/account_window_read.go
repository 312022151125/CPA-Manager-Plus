...
func (r *repository) LoadAccountWindowStats(ctx context.Context, windows []AccountWindowUsageQuery) ([]AccountWindowModelStat, State, bool, error) {
	if len(windows) == 0 { return []AccountWindowModelStat{}, State{}, true, nil }
	tx, err := r.db.BeginTx(ctx, &sql.TxOptions{ReadOnly: true})
	if err != nil { return nil, State{}, false, err }
	defer func() { _ = tx.Rollback() }()
	state, available, projectionComplete, err := projectionReadState(ctx, tx)
	if err != nil || !available { return nil, state, available, err }
	statsState, revision, dailyAvailable, err := statsReadState(ctx, tx)
	if err != nil { return nil, state, false, err }
	grouped := make(map[accountWindowStatKey]*AccountWindowModelStat)
	if dailyAvailable {
		if err := mergeStoredAccountWindowStats(ctx, tx, revision, windows, grouped); err != nil { return nil, state, false, err }
	}
	if err := mergeProjectedAccountWindowStats(ctx, tx, windows, state.CoverageEventID, projectionComplete, statsState.CoverageEventID, dailyAvailable, grouped); err != nil { return nil, state, false, err }
	if err := tx.Commit(); err != nil { return nil, state, false, err }
	return sortedAccountWindowStats(grouped), state, true, nil
}

type accountWindowStatKey struct { requestIndex int; model string; billingModel string; pricingModel string; contextThreshold int64; serviceTier string }
...
func mergeStoredAccountWindowStats(ctx context.Context, tx *sql.Tx, revision string, windows []AccountWindowUsageQuery, grouped map[accountWindowStatKey]*AccountWindowModelStat) error {
	values := make([]string, 0, len(windows)); args := make([]any, 0, len(windows)*5+1)
	for _, window := range windows {
		if !accountWindowCanUseDaily(window) { continue }
		fullStartMS := ceilDayMS(window.FromMS); fullEndMS := floorDayMS(window.ToMS)
		if fullStartMS >= fullEndMS { continue }
	...
	if err != nil { return err }
	defer rows.Close(); return mergeAccountWindowStatRows(rows, grouped)
}
...
func mergeAccountWindowStatRows(rows *sql.Rows, grouped map[accountWindowStatKey]*AccountWindowModelStat) error {
	for rows.Next() {
		...
		current := grouped[key]
		if current == nil { copy := stat; grouped[key] = &copy; continue }
		current.Calls += stat.Calls; current.SuccessCalls += stat.SuccessCalls; current.FailureCalls += stat.FailureCalls
		current.InputTokens += stat.InputTokens; current.OutputTokens += stat.OutputTokens; current.CachedTokens += stat.CachedTokens
		current.CacheReadTokens += stat.CacheReadTokens; current.CacheCreationTokens += stat.CacheCreationTokens
		current.LongInputTokens += stat.LongInputTokens; current.LongOutputTokens += stat.LongOutputTokens; current.LongCachedTokens += stat.LongCachedTokens
		current.LongCacheReadTokens += stat.LongCacheReadTokens; current.LongCacheCreationTokens += stat.LongCacheCreationTokens
		current.TotalTokens += stat.TotalTokens; current.LastSeenMS = max(current.LastSeenMS, stat.LastSeenMS)
	}
	return rows.Err()
}

func sortedAccountWindowStats(grouped map[accountWindowStatKey]*AccountWindowModelStat) []AccountWindowModelStat {
	stats := make([]AccountWindowModelStat, 0, len(grouped)); for _, stat := range grouped { stats = append(stats, *stat) }
	sort.Slice(stats, func(i, j int) bool {
		if stats[i].RequestIndex != stats[j].RequestIndex { return stats[i].RequestIndex < stats[j].RequestIndex }
		if stats[i].LastSeenMS != stats[j].LastSeenMS { return stats[i].LastSeenMS > stats[j].LastSeenMS }
		if stats[i].Model != stats[j].Model { return stats[i].Model < stats[j].Model }
		if stats[i].BillingModel != stats[j].BillingModel { return stats[i].BillingModel < stats[j].BillingModel }
		if stats[i].PricingModel != stats[j].PricingModel { return stats[i].PricingModel < stats[j].PricingModel }
		if stats[i].ContextThresholdTokens != stats[j].ContextThresholdTokens { return stats[i].ContextThresholdTokens < stats[j].ContextThresholdTokens }
...
func accountWindowEventSourceSQL(windows []AccountWindowUsageQuery, coverageEventID int64, projectionComplete bool, statsCoverageEventID int64, dailyAvailable bool) (string, []any) {
	values := make([]string, 0, len(windows)); args := make([]any, 0, len(windows)*8+4)
	for _, window := range windows {
		accountKey, legacyAccountKey := accountWindowKeys(window); values = append(values, "(?, ?, ?, ?, ?, ?, ?, ?)")
	...
	if dailyAvailable { query += ` and (w.use_daily = 0 or p.timestamp_ms < w.full_start_ms or p.timestamp_ms >= w.full_end_ms or p.event_id > ?)`; args = append(args, statsCoverageEventID) }
	if projectionComplete { return query, args }
	...
	args = append(args, coverageEventID)
	if dailyAvailable { query += ` and (w.use_daily = 0 or e.timestamp_ms < w.full_start_ms or e.timestamp_ms >= w.full_end_ms or e.id > ?)`; args = append(args, statsCoverageEventID) }
	return query, args
}

func accountWindowCanUseDaily(window AccountWindowUsageQuery) bool {
	authFile := strings.TrimSpace(window.AuthFileSnapshot); authIndex := strings.TrimSpace(window.AuthIndex)
	if authFile == "" || authIndex == "" { return false }
